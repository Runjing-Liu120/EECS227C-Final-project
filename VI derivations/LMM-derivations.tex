...
y_n | u_g, \beta &\sim \mathcal N (x_n^T\beta + \mu_{g(n)}, \sigma^2_y)\quad \text{for $n = 1, ..., N$}
\end{align}

\section{Variational distributions}
We will take a fully factorized distrubtion over $\beta$ and $\{u_g\}_{g=1}^{N_G}$. In particular, 
\begin{align}
...
\end{align*}
For a chose $g\in\{1, ..., N_G\}$, we seek the updates for $\mu_g$. We take expectations of the log likelihood with respect to $\beta$ and $\mu_i$ $i\not=g$, dropping constants independent of $\mu_g$: 
\begin{align*}
E_{\beta, \mu_{(-g)}} [ \log p(y, \mu, \beta | X) ] &=  - \frac{1}{2\sigma^2_\mu} \mu_g^2 - \frac{1}{2\sigma^2_y}\sum_{n: g(n) = g} E_{\beta, \mu_{(-g)}}[(y_n - x_n^T\beta - \mu_{g})^2] + K\\
...
\begin{align*}
 ...
 &= - \frac{1}{2\sigma^2_y}\sum_{n=1}^N \Big((\tau^2_{\mu_{g(n)}} + \hat\mu_{g(n)}^2) - 2\hat\mu_{g(n)}(y_n - x_n^T\hat\beta)\Big) - \frac{1}{2\sigma^2_y} \hat\beta^TXX^T\hat\beta + \frac{1}{2\sigma^2_y} y^TX^T\hat\beta
\end{align*}
...
\begin{align*}
\mathcal L = -\frac{1}{2\sigma^2_\beta} (\text{Tr}(\Sigma_\beta) + \hat \beta^T \hat\beta)- &\frac{1}{2\sigma^2_\mu}\sum_{g=1}^{N_G}(\tau^2_{\mu_g} + \hat\mu_g^2)- \frac{1}{2\sigma^2_y}\sum_{n=1}^N \Big((\tau^2_{\mu_{g(n)}} + \hat\mu_{g(n)}^2) - 2\hat\mu_{g(n)}(y_n - x_n^T\hat\beta)\Big) - \frac{1}{2\sigma^2_y} \hat\beta^TXX^T\hat\beta + \frac{1}{2\sigma^2_y} y^TX^T\hat\beta\\
&+\sum_{g=1}^{N_G}\frac{1}{2} \log(2\pi e \sigma_{\mu_g}^2) + \frac{1}{2} \log((2\pi e)^k |\Sigma_{\beta}|)
\end{align*}
