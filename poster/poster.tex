...
\begin{itemize}
\item {\bf coordinate ascent}, the standard method for variational inference;
\item {\bf Newton conjugate trust region}, an accepted second-order optimization method; 
\item and {\bf parameter expanded variational inference}, a data-augementation method described in Qi and Jaakkola 2006. 
\end{itemize}
...
Let $t_n, n=1, ..., N$ be binary random variables. In probit regression our model is
\begin{align*}
p(t | X, w) = \prod_{n=1}^N \Phi(x_n^Tw)
\end{align*}
where $\Phi$ is the standard normal c.d.f function. The $x_n$'s are fixed, and we place a normal prior on $w$. To do inference on $w$, we take a variational approach, optimizing over variational parameters to maximize the evidence lower bound. The results of CAVI, Newton-CG, and PX-VB are displayed below.   
...
\begin{figure}[h]
  \centering
    ...
    \end{subfigure}
    \caption{Convergence results for synthetic data. Number of data points was  $N=500$, dimension of $w$ was $D=20$. }
\end{figure}
...
\begin{center}
\begin{tabular}{ccc}
\includegraphics[width=0.7\linewidth]{logo.png} %& \hfill & \includegraphics[width=0.4\linewidth]{logo.png}
\end{tabular}
\end{center}
